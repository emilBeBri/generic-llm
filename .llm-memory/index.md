...
- [CONVENTIONS-usage-cost-emission.md](CONVENTIONS-usage-cost-emission.md) — `--usage` emits one `gllm-usage {json}` line to stderr (tokens/cache/reasoning + provider-verbatim `usage_raw` + derived `cost_usd`/`priced_as`); per-provider mappers in `usage.py` normalise usage at the source; `pricing.py` converts to USD from the 24h-cached llm-prices.com feed with provider-aware billing (GLM has no feed price).
