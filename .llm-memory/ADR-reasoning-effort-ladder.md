...
- **Token/temperature side-effects when a level is set:** the output budget is raised so reasoning doesn't starve the answer — but **no longer by the adapters**; `cli._resolve_max_tokens` does it once for every provider and honours an explicit `--max-tokens` instead of overriding it (see [[ADR-output-budget-resolution]]). Adapters still **drop `temperature`** (reasoning models reject a custom one). The direct Anthropic adapter also switches to `messages.stream()` when thinking is on (long generations outrun the non-streaming socket timeout). Gemini keeps temperature.
